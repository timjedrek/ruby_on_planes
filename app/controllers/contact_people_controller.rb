class ContactPeopleController < ApplicationController
  ...
  def set_school
    ...
    begin
      ...
      if @school.nil?
        ...
        render json: { success: false, message: "School not found with slug or ID: #{params[:school_id]}" }, status: :not_found
      end
    rescue => e
      Rails.logger.error "Error in set_school: #{e.message}"
      Rails.logger.error e.backtrace.join("\n")
      render json: { success: false, message: "Error finding school or airport: #{e.message}" }, status: :not_found
    end
  end
  ...
  def set_contact_person
    begin
      ...
      render json: { success: false, message: "Contact person not found" }, status: :not_found
    end
  end
  ...
  def contact_person_params
    begin
      ...
      render json: { success: false, message: "Contact person parameters are missing" }, status: :bad_request
    end
  end
